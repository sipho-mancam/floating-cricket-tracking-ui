...
class VizControls(QWidget):
    def __init__(self, evnts_controller:EventsController,  parent = None):
        super().__init__(parent)
        self.__events_controller = evnts_controller
        self.__main_layout = QVBoxLayout()
        self.__viz_event_sender = VizEventSender()
        self.initUI()
        self.setLayout(self.__main_layout)

    def animate_in_handler(self)->None:
        evt = self.__events_controller._build_control_event_object("animate_in", {})
        self.__viz_event_sender.send_event(evt, "CONTROL_OBJECT")
        
    def animate_out_handler(self)->None:
        evt = self.__events_controller._build_control_event_object("animate_out", {})
        # self.__events_controller.send_current_event()
        self.__viz_event_sender.send_event(evt, "CONTROL_OBJECT")


    def switch_ends_handler(self)->None:
        evt = self.__events_controller._build_control_event_object("switch_ends", {})
        self.__viz_event_sender.send_event(evt, "CONTROL_OBJECT")

    def switch_leg_handler(self)->None:
        evt = self.__events_controller._build_control_event_object("switch_leg", {})
        self.__viz_event_sender.send_event(evt, "CONTROL_OBJECT")

    def initUI(self)->None:
        self.__title_text = QLabel("Viz Controls")
        self.__title_text.setObjectName("viz_controls_title")
        self.__main_layout.addWidget(self.__title_text, alignment=Qt.AlignHCenter)

        buttons_layout = QHBoxLayout()
        self.__animate_in_btn = QPushButton("Animate In")
        self.__animate_in_btn.clicked.connect(self.animate_in_handler)
        self.__animate_out_btn = QPushButton("Animate Out")
        self.__animate_out_btn.clicked.connect(self.animate_out_handler)

        self.__preview_btn = QPushButton("Preview")
        buttons_layout.addWidget(self.__animate_in_btn)
        buttons_layout.addWidget(self.__animate_out_btn)
        buttons_layout.addWidget(self.__preview_btn)
        self.__main_layout.addLayout(buttons_layout)

        self.__map_controls_title = QLabel("Mini Map Controls")
        self.__map_controls_title.setObjectName("viz_controls_title")
        self.__main_layout.addWidget(self.__map_controls_title, alignment=Qt.AlignHCenter)
        map_controls_layout = QHBoxLayout()

        self.__switch_ends = QPushButton("Switch Ends")
        self.__switch_ends.clicked.connect(self.switch_ends_handler)
        self.__switch_sides = QPushButton("Switch Leg Side")
        self.__switch_sides.clicked.connect(self.switch_leg_handler)
        map_controls_layout.addWidget(self.__switch_ends)
        map_controls_layout.addWidget(self.__switch_sides)
        self.__main_layout.addLayout(map_controls_layout)
...
class CricketTrackingWidget(QWidget):
    def __init__(self, controller, parent = None):
        ...
        self.__viz_controls = VizControls(self.__events_controller)
    ...
    def initUI(self)->None:
        ...
        self.__main_layout.addWidget(self.__viz_controls, 1, 1, alignment=Qt.AlignTop)
